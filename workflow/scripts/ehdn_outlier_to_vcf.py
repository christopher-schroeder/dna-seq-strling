...
def convert(filename, fileoutput, cases):
    with open(filename, "r") as f, open(fileoutput, "w") as o:
        ...
        for i, line in enumerate(f):
            ...
            #     continue
            case_counts = dict(map(lambda x: x.split(":"), l.high_case_counts.split(",")))
            if len(case_counts) < 2:
                continue

            counts = Counter(l.counts.split(","))

            p_value = scipy.stats.norm.sf(abs(float(l.top_case_zscore)))
            if p_value == 0.0:
                qual = 255
            else:
                qual = min(255, -10 * math.log10(float(p_value)))

            control_counts = counts - Counter(case_counts.values()) # remove case counts from counts
            control_counts = reduce(lambda x, y: x + y, [[n] * count for n, count in control_counts.items()], [])
            control_counts = ",".join(control_counts)
            case_counts = "\t".join([f"{case_counts[c]}:.:." if c in case_counts else f".:.:." for c in cases])
